            ...
            for row in chunk:
                if row != None and row != '':
                    ...
                    if event_type == "message":
                      obj_arraytemp.append(y)
                      df = pd.DataFrame(obj_arraytemp)
                      df_explode = df.assign(var1=df['msgParts_s'].str.split(',')).explode('msgParts_s')
                      df_explode.to_json(orient='records')
                      obj_arraytemp.clear()
                      obj_array.append(df_explode)
                      print(obj_array)
                  else:
                     obj_array.append(y)
